                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">@lang('site.subjects')</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
        ...
        <section class="content">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">@lang('site.subjects')<small>{{ $subjects->total() }}</small></h3><br>
                    <form action="{{ route('dashboard.subjects.index') }}" method="get">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                ...
                                @if(auth()->user()->hasPermission('subjects_create'))
                                    <a href="{{ route('dashboard.subjects.create') }}" class="btn btn-info"><i class="fa fa-plus"></i> @lang('site.add')</a>
                                @else
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            @foreach($subjects as $index=>$subject)
                                <tr>
                                    ...
                                    <td>{{ $subject->name }}</td>
                                    <td>
                                        @if(auth()->user()->hasPermission('subjects_update'))
                                        <a href="{{ url('dashboard/subjects/' . $subject->id . '/edit') }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> @lang('site.edit')</a>
                                        @else
                                            <button class="btn btn-info disabled">@lang('site.update')</button>
                                        @endif
                                        @if(auth()->user()->hasPermission('subjects_delete'))
                                        <form action="{{ route('dashboard.subjects.delete', $subject->id) }}" method="post" style="display: inline-block">
                                            @csrf
                                            @method('DELETE')
