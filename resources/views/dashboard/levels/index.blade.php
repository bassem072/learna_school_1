            ...
            <nav class="mt-2">
                <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                    ...
                    @if(auth()->user()->hasPermission('levels_read'))
                        <li class="nav-item">
                            <a href="{{ route('dashboard.levels.index') }}" class="nav-link active">
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">@lang('site.levels')</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
        ...
        <section class="content">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">@lang('site.levels')<small>{{ $levels->total() }}</small></h3><br>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                ...
                                @if(auth()->user()->hasPermission('levels_create'))
                                    <a href="{{ route('dashboard.levels.create') }}" class="btn btn-info"><i class="fa fa-plus"></i> @lang('site.add')</a>
                                @else
                    ...
                    @if($levels -> count() > 0)
                        <table class="table table-bordered table-hover">
                            ...
                            <tr>
                                ...
                                <th>@lang('site.education_system')</th>
                            ...
                            @foreach($levels as $index=>$level)
                                <tr>
                                    ...
                                    <td>{{ $level->year->name }}</td>
                                    <td>{{ $level->section->name }}</td>
                                    <td>{{ $level->education_system->name }}</td>
                                    <td>
                                        @if(auth()->user()->hasPermission('levels_update'))
                                        <a href="{{ url('dashboard/levels/' . $level->id . '/edit') }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> @lang('site.edit')</a>
                                        @else
                                            <button class="btn btn-info disabled">@lang('site.update')</button>
                                        @endif
                                        @if(auth()->user()->hasPermission('levels_delete'))
                                        <form action="{{ route('dashboard.levels.delete', $level->id) }}" method="post" style="display: inline-block">
                                            @csrf
                                            @method('DELETE')
