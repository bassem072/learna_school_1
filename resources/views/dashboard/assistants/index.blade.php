        ...
        <section class="content">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">@lang('site.assistants')<small>{{ $assistants->total() }}</small></h3><br>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                ...
                                @if(auth()->user()->hasPermission('assistants_create'))
                                    <a href="{{ route('dashboard.assistants.create') }}" class="btn btn-info"><i class="fa fa-plus"></i> @lang('site.add')</a>
                                @else
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            @foreach($assistants as $index=>$assistant)
                                <tr>
                                    ...
                                    <td>{{ $assistant->assistant_profile->description }}</td>
                                    <td>{{ $assistant->assistant_profile->teacher->first_name }}</td>
                                    <td><img src="{{ $assistant->getImagePathAttribute() }}" alt="@lang('site.image_profile')" style="height: 50px;" class="img-thumbnail"></td>
                                    <td>
                                        @if(auth()->user()->hasPermission('assistants_update'))
                                        <a href="{{ url('dashboard/assistants/' . $assistant->id . '/edit') }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> @lang('site.edit')</a>
                                        @else
                                            <button class="btn btn-info disabled">@lang('site.update')</button>
                                        @endif
                                        @if(auth()->user()->hasPermission('assistants_delete'))
                                        <form action="{{ route('dashboard.assistants.delete', $assistant->id) }}" method="post" style="display: inline-block">
                                            @csrf
                                            @method('DELETE')
