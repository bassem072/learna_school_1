            ...
            <nav class="mt-2">
                <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                    ...
                    @if(auth()->user()->hasPermission('assistants_read'))
                        <li class="nav-item">
                            <a href="{{ route('dashboard.assistants.index') }}" class="nav-link active">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('dashboard.assistants.index') }}">@lang('site.assistants')</a></li>
        ...
        <section class="content">
            <div class="card card-primary">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('dashboard.assistants.update', $assistant->id) }}" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf

                        <div class="form-group">
                            <label>@lang('site.first_name')</label>
                            <input type="text" name="first_name" class="form-control" value="{{ $assistant->first_name }}">
                        </div>

                        <div class="form-group">
                            <label>@lang('site.last_name')</label>
                            <input type="text" name="last_name" class="form-control" value="{{ $assistant->last_name }}">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="description" placeholder="@lang('site.description')"></textarea>
                        </div>

                        <div class="form-group">
                            <label>@lang('site.teachers')</label>
                            <select class="form-control" name="teacher_id">
                                <option value="">@lang('site.select_teacher')</option>
                                @foreach($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" {{ $teacher->id == $assistant->assistant_profile->teacher->id ? 'selected' : '' }}>{{ $teacher->first_name }} {{ $teacher->last_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="col-sm-2 imgUp">
                                    <div class="imagePreview" style="background-image: url({{ asset('uploads/user_images/' . $assistant->image) }})"></div>
                                    <label class="btn btn-primary button-image">
                                        Upload<input type="file" name="image" class="uploadFile img" accept="image/*" style="width: 0;height: 0;overflow: hidden;">
                                    </label>
                                </div><!-- col-2 -->
                            </div><!-- row -->
                        </div><!-- container -->
                        ...
                        <div class="form-group">
                            ...
                            <div class="card">
                                <div class="card-header d-flex p-0">
                                    @php
                                        $models = ['assistants', 'subjects', 'levels', 'terms'];
                                    @endphp
                                                ...
                                                <div class="row">
                                                    ...
                                                    @foreach($maps as $index1=>$map)
                                                        <div class="form-check col-md-3">
                                                            <input type="checkbox" class="form-check-input" name="permissions[]" {{ $assistant->hasPermission($model . '_' . $map) ? 'checked' : '' }} value="{{$model . '_' . $map}}" id="exampleCheck{{$index . $index1}}">
