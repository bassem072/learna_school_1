...
(function (factory) {
    ...
    Mapael.prototype = {
        ...
        drawLink: function (id, xa, ya, xb, yb, elemOptions) {
            ...
            // dashboard_files(c,d) = dashboard_files(a,b) (=abDist)
            var abDist = Math.sqrt((xb - xa) * (xb - xa) + (yb - ya) * (yb - ya));

            // Solution for equation dashboard_files(cd) = sqrt((xd - xc)² + (yd - yc)²)
            // dashboard_files(c,d)² = (xd - xc)² + (yd - yc)²
            // We assume that dashboard_files(c,d) = dashboard_files(a,b)
            // so : (xd - xc)² + (yd - yc)² - dashboard_files(a,b)² = 0
            // With the factor : (xd - xc)² + (yd - yc)² - (factor*dashboard_files(a,b))² = 0
            // (xd - xc)² + (acd*xd + bcd - yc)² - (factor*dashboard_files(a,b))² = 0
            var a = 1 + acd * acd;
